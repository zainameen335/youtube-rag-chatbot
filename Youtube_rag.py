...
def get_transcript(video_id):
    ...
    try:
        ...
        api_key = os.getenv("SUPADATA_KEY") or st.secrets.get("SUPADATA_KEY", None)
        if not api_key:
            st.error("Supadata API key not found in secrets.")
            return None
        ...
        if response.status_code == 200:
            data = response.json()
            transcript = [
                {"text": chunk["text"], "start": chunk["offset"] / 1000}
            ...
            st.error("Invalid Supadata API key.")
            return None
        else:
...
if st.session_state.video_loaded:
    ...
    question = st.text_input(
        "Ask your question:",
        key=f"question_input_{st.session_state.video_id}"
    )
